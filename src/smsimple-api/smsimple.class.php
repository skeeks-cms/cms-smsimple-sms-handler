 ...
 */
$LIB_EXISTS = include('./lib/xmlrpc.inc');
...
class SMSimple
{
    ...
    /**
     * Системный метод вызова XML-RPC функций API (осуществляет выбор варианта, затем вызов этого варианта)
     */
     ...
     * 
     * @param:  $params  array()  Массив параметров, обязательные: `groups`, `template`; опциональные: `title`, `origin_id`, `start_date`, `start_time`, `stop_time`.
     *
     ...
     *
     * @param_element*:  $params  $params['title']  string  называние рассылки (для Вас), это название будет фигурировать только в списке рассылок в ЛК, если его на задать, оно будет создано автоматически
     ...
     * @param_element*:  $params  $params['start_date']  string  дата старта рассылки - дата в формате *YYYY-MM-DD* задаёт день, в который рассылка будет отправлена (если значение `start_date` не задано, то оно будет принятj равным текущему дню)
     ...
     *   
     *   1. создание новой группы контактов
     *   2. импортирование в эту группу XLS файла со столбцами "Телефон", "Текст для оправки на этот телефон" в поля "телефон " и "доп. поле 1", соответственно.
     *   3. создание рассылки по этой группе с текстом сообщения "%custom_1%"
     * 
     * @param:  $params  array()  Массив параметров, обязательные: `messages`; опциональные: `title`, `origin_id`, `start_date`, `start_time`, `stop_time`, `delete_groups`.
     *
     * @param_element:  $params  $params['messages']  array()  индексированный массив (список) с элементами вида `array('phone' => '79991234567', 'text' => 'Текст, который будет отправлен на номер 79991234567')`
     *
     * @param_element*:  $params  $params['title']  string  называние рассылки (для Вас), это название будет фигурировать только в списке рассылок в ЛК, если его на задать, оно будет создано автоматически
     * @param_element*:  $params  $params['origin_id']  int  номер подписи (номера можно узнать в личном кабинете на странице ["Подписи"](https://smsimple.ru/origins)); если этот параметр отсутствует, будет взята подпись, заданная как "Основная" в ЛК
     ...
     *                                                  Это время будет использовано только в случае, когда рассылка не успела завершиться. Можно рассматривать его как "аварийное" время завершения.
     *                                                  Это может быть важно по ряду причин, в частности, информация рассылки может потерять актуальность, или время рассылки слишком позднее (ночь).
     * @param_element*:  $params  $params['delete_groups']  int  если передать 1, то группа, которая была автоматически создана под эту рассылку, будет удалена сразу по окончании рассылки,
                                                            иначе (если передать 0) - группа будет сохранена (если параметр отсутствует, передаётся 0, группа будет сохранена) 
     *
     * @returns:  int  Возвращает номер (id) новой рассылки.
     */
    public function addMultisendJob($params=array()) {
        return $this->_doApiCall('pajm.job.multisend', array(
            'session_id' => $this->session_id,
            'origin_id'  => $params['origin_id'],
            'title'      => $params['title'],
            'messages'   => $params['messages'],
            'delete_groups' => $params['delete_groups'] ? 1 : 0
        ));
    }
     ...
     *
     * @returns:  array()  Возвращает контакты из группы в виде массива массивов
     * 
     *     array(
     *           array(
     *                 'id' => 1,
     ...
     *
     * @param:  $year_begin   int   год начала периода;
     * @param:  $month_begin   int   месяц начала периода;
     * @param:  $day_begin   int   день начала периода;
     * @param:  $year_end   int   год конца периода;
     * @param:  $month_end   int   месяц конца периода;
     * @param:  $day_end   int   день конца периода;
     * @param*:  $count   int   если нужно только количество, передаётся значение `true`;
     * @param*:  $limit   int   максимальная длина выборки (по аналогии с SQL-командой `LIMIT`); по-умолчанию `0`, что означает "не ограничивать";
     * @param*:  $offset   int   начальный отступ выборки (по аналогии с SQL-командой `OFFSET`); по-умолчанию `0`;
     *
     * @returns:  array()  Возвращает отправленные смс за выбранный период в виде массива массивов
     * 
     *     array(
     *           array(
     *                 "origin" => "MyOrigin",
     ...
     */
    public function sms_statistic($year_begin, $month_begin, $day_begin, $year_end, $month_end, $day_end, $count=false, $limit=0, $offset=0) {
